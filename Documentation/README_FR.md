...
| **System Configuration** | *["R58_network_package_list.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/6-system_configuration/6.5-package_management/R58_network_package_list.yml)* | Ce script Yaml est utilisé pour installer plusieurs outils de réseau sur un système Linux en utilisant le gestionnaire de paquets dnf. Il met à jour ou installe les paquets suivants : nmap pour l'analyse réseau, tcpdump pour la capture de paquets, wireshark pour l'analyse de paquets, net-tools pour des utilitaires réseau classiques, et traceroute pour tracer les routes réseau. Le playbook assure que ces outils sont à jour et disponibles pour les tâches de diagnostic et d'analyse réseau. |
| **System Configuration** | *["R58_office_package_list.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/6-system_configuration/6.5-package_management/R58_office_package_list.yml)* | Ce script Yaml automatise l'installation de plusieurs applications de bureau sur un système utilisant le gestionnaire de paquets dnf. Il met à jour ou installe les versions les plus récentes de LibreOffice, Firefox, Thunderbird, GIMP, et Inkscape. En résumé, il permet de déployer rapidement un ensemble d'outils bureautiques et graphiques essentiels sur des machines ciblées. |
| **System Configuration** | *["R59_trusted_repo.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/6-system_configuration/6.5-package_management/R59_trusted_repo.yml)* | Ce script Yaml est conçu pour configurer les dépôts de paquets sur un système AlmaLinux. Il active le dépôt EPEL, ajoute les dépôts de base d'AlmaLinux (BaseOS, AppStream et Extras) avec vérification des signatures GPG pour assurer leur intégrité, et supprime les anciens dépôts non fiables. Enfin, il met à jour tous les paquets installés à leur dernière version disponible. En résumé, il vise à garantir que le système utilise les sources de paquets officielles et à jour. |
| **System Configuration** | *["R61_auto_updates.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/6-system_configuration/R61_auto_updates.yml)* | Ce script Yaml a pour but de configurer et d'automatiser les mises à jour des packages sur un système Linux utilisant DNF. Il installe le paquet dnf-automatic, active et démarre le timer pour les mises à jour automatiques, puis configure le fichier de configuration de dnf-automatic pour appliquer automatiquement les mises à jour disponibles, émettre des messages à chaque mise à jour et définir les intervalles entre les vérifications et les mises à jour. En gros, il permet de garder le système à jour sans intervention manuelle. |
| **System Configuration** | *["clean_files.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/6-system_configuration/6.5-package_management/clean_files.yml)* | Ce script Yaml utilise le module dnf pour nettoyer automatiquement les paquets installés en tant que dépendances qui ne sont plus nécessaires. En d'autres termes, il supprime les paquets orphelins ou inutilisés qui ont été installés comme dépendances mais ne sont plus requis par aucun autre logiciel, ce qui permet de libérer de l'espace et de garder le système plus propre. |
| **System Configuration** | *["full_update.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/6-system_configuration/6.5-package_management/full_update.yml)* | Ce script Yaml est conçu pour mettre à jour tous les paquets installés sur un système utilisant le gestionnaire de paquets dnf (souvent trouvé sur les distributions basées sur Fedora). En spécifiant name: "*" et state: latest, il met à jour tous les paquets vers leur dernière version disponible. L'option ignore_errors: true permet de continuer l'exécution du playbook même si certaines erreurs surviennent lors de la mise à jour. En gros, il assure que tous les paquets sont à jour tout en étant tolérant aux éventuelles erreurs. |
| ~~~ | ~~~ | ~~~ |
| **Services Configuration** | *["R62_disable_unnecessary_services.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/7-services_configuration/R62_disable_unnecessary_services.yml)* | Ce script Yaml sert à désactiver et arrêter plusieurs services jugés inutiles sur un système. Il inclut des services comme avahi-daemon (pour la découverte de services réseau), bluetooth, cups (impression), ModemManager (pour la connectivité modem), postfix (serveur de mail), et rpcbind (appel de procédure distante). En les arrêtant et en les désactivant, on vise à améliorer la sécurité et les performances du système en réduisant la surface d'attaque et les ressources consommées. |
| **Services Configuration** | *["R65-R66-R78_containerization.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/7-services_configuration/R65-R66-R78_containerization.yml)* | Ce script Yaml sert à installer et configurer Docker sur un système basé sur CentOS. Il commence par installer les dépendances nécessaires, puis ajoute le dépôt Docker et installe Docker ainsi que ses composants essentiels. Ensuite, il démarre et active le service Docker, crée un répertoire de configuration, et applique des options de durcissement pour améliorer la sécurité et les performances de Docker. Enfin, il redémarre Docker pour que les nouvelles configurations prennent effet. |
| **Services Configuration** | *["R69_openssh_configuration.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/7-services_configuration/R69_openssh_configuration.yml)* | Ce script Yaml configure un serveur SSH en installant le package OpenSSH, en sauvegardant le fichier de configuration actuel (sshd_config), et en appliquant plusieurs modifications de sécurité. Il désactive la connexion root, active l'authentification par clé publique et désactive l'authentification par mot de passe. Enfin, il redémarre le service SSH pour appliquer ces changements. |
| **Services Configuration** | *["R73_Configure_auditd.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/7-services_configuration/R73_Configure_auditd.yml)* | Ce script Yaml sert à configurer le système d'audit sur une machine Linux. Il installe le paquet auditd, s'assure que le service auditd est actif et démarré, et met en place des règles d'audit pour surveiller des fichiers et des événements spécifiques, comme les modifications aux fichiers de mot de passe et aux journaux. Les règles définies garantissent une surveillance continue des accès et des modifications critiques pour renforcer la sécurité du système. |
| **Services Configuration** | *["R80_reduce_network_attack.yml"](https://github.com/ArtemissFR/DEIMOS_Project/blob/main/roles/ANSSI_Configuration/tasks/7-services_configuration/R80_reduce_network_attack.yml)* | Ce script Yaml configure le pare-feu UFW (Uncomplicated Firewall) sur un serveur. Il installe UFW si ce n'est pas déjà fait, l'active, et définit les règles par défaut pour bloquer les connexions entrantes tout en permettant les connexions sortantes. Il autorise ensuite les connexions SSH, ainsi que le trafic HTTP et HTTPS, en fonction du serveur web utilisé (Apache ou Nginx). Enfin, il recharge UFW pour appliquer les changements. |
